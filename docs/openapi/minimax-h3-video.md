...
提示词增强是平台可选能力。平台启用后，省略该字段或传 `true` 会尝试增强提示词；若需要严格按原始提示词提交，可增加：

```json
"prompt_enhance": false
```

multipart 请求使用 `--form "prompt_enhance=false"`。该参数只接受 `true` 或 `false`（也兼容 JSON 字符串形式）；其他值会返回参数错误。平台未启用增强、增强过程临时不可用或增强失败时，服务会自动回退原始 `prompt` 继续创建任务。响应不会单独标识本次是否实际完成增强；只有显式传 `false` 才能确保不进行增强。
...
首尾帧模式需要显式传入 `mode: "first_last_frame"`，并且必须提供恰好两张图片。第一张图片作为视频首帧，第二张图片作为视频尾帧；系统会生成两帧之间的连续过渡视频。

普通多参考图请求不要传该 `mode`。即使普通请求也传了两张图片，未指定 `first_last_frame` 时仍按多参考图生成，不会自动改成首尾帧。
...
curl --request POST "http://103.36.63.156:3000/v1/videos" \
  ...
  --data '{
    ...
    "prompt": "镜头平稳推进，人物从首帧姿态自然过渡到尾帧姿态，动作连续，保持人物身份和场景一致",
    "seconds": 5,
    "size": "1376x768",
    "images": [
...
curl --request POST "http://103.36.63.156:3000/v1/videos" \
  --header "Authorization: Bearer $API_KEY" \
  --form "model=minimax_h3" \
  --form "mode=first_last_frame" \
  --form "prompt=从首帧自然运动到尾帧，保持主体外观稳定，镜头运动平滑" \
  --form "seconds=5" \
  --form "size=1376x768" \
  --form "images=@./first-frame.png" \
  --form "images=@./last-frame.png"
```

首尾帧模式不接受参考视频、参考音频或视频配套音频。图片少于两张或多于两张会直接返回参数错误，并且不会提交生成任务。
...
| `mode` | string | 否 | 首尾帧模式传 `first_last_frame`；省略时保持普通文生视频或多参考图模式。 |
| `prompt` | string | 是 | 视频提示词，不能为空。建议描述主体、动作、场景、镜头和风格。 |
| `prompt_enhance` | boolean | 否 | 提示词增强开关。平台启用后，省略字段或传 `true` 会尝试增强；显式传 `false` 时直接使用原始 `prompt`。JSON 与 multipart 均支持；平台未启用、增强不可用或增强失败时自动回退原始提示词，不会仅因增强失败而让视频任务失败。 |
...
中转站不会直接使用下游提交的图片、视频或音频 URL。素材处理过程如下：
...
建议使用可稳定访问、有效期覆盖整个生成过程的 HTTPS URL。中转站不对素材做格式转换；请上传常见的标准图片、视频和音频文件。素材下载或解析失败时，任务会返回 `failed` 或创建错误。
...
| `metadata.url` | string | 仅任务成功时有意义。它是可直接在浏览器或播放器中打开的成片地址，带短期签名，默认有效 24 小时；过期后请使用 API Key 重新查询任务以取得新地址。 |
| `error.message` | string | 仅 `failed` 时出现，描述失败原因。 |

任务 ID 按中转站账号隔离。查询时应使用创建该任务的同一账号下的 API Key；不要把任务 ID 当作公开的视频链接分发。`metadata.url` 是短期访问凭证，不含 API Key，但在有效期内持有该地址即可播放或下载，因此同样不应公开传播。
...
`metadata.url` 是成片的短期签名结果 URL，可直接赋给浏览器的 `<video src>`、播放器或下载器，无需额外添加 `Authorization` 请求头。它默认有效 24 小时；过期后重新查询同一任务即可取得新的地址。

若希望始终使用 API Key 获取内容流，可调用下列稳定下载接口。该接口支持播放器常用的 HTTP `Range` 分段读取和断点续传：
...
下表为 `minimax_h3` 的公开统一价格。总基础费用 = **对应清晰度单价 × 视频秒数**；实际像素尺寸请参见第 5.2.1 节。

| 清晰度档位 | 公开价格（元/秒） | 计价标准 |
| --- | ---: | --- |
| `0.2 MP` | `0.022` | `0.10 × 0.2 × 1.1` |
| `0.3 MP` | `0.033` | `0.10 × 0.3 × 1.1` |
| `0.4 MP` | `0.044` | `0.10 × 0.4 × 1.1` |
| `0.5 MP` | `0.055` | `0.10 × 0.5 × 1.1` |
| `0.6 MP` | `0.066` | `0.10 × 0.6 × 1.1` |
| `0.7 MP` | `0.077` | `0.10 × 0.7 × 1.1` |
| `0.8 MP` | `0.088` | `0.10 × 0.8 × 1.1` |
| `0.9 MP` | `0.099` | `0.10 × 0.9 × 1.1` |
| `0.98 MP` | `0.1078` | `0.10 × 0.98 × 1.1` |
| `1.0 MP`（768P 基准档） | `0.10` | 固定基准价格 |
| `1.2 MP` | `0.18` | `0.10 × 1.2 × 1.5` |
| `1.5 MP` | `0.225` | `0.10 × 1.5 × 1.5` |
| `1.8 MP` | `0.27` | `0.10 × 1.8 × 1.5` |
| `2.0 MP`（16:9 为 1080P 档） | `0.30` | `0.10 × 2.0 × 1.5` |

图生视频的前 5 张参考图片包含在该任务的基础价格中；从第 6 张开始，每多 1 张额外加收 **0.10 元/张**。参考视频、参考视频配套音频、参考音频和提示词增强开关本身不单独按条计费。

例如，10 秒、2.0 MP 的请求基础费用为 `0.30 × 10 = 3.00 元`。中转站的余额、额度和日志可能以 USD 额度展示；系统会按站点设置的汇率将人民币价格换算为额度，请以任务创建时的账单记录为准。
...
| `at most 9 reference images` / `at most 3 reference videos` / `at most 3 reference video audio files` / `at most 3 reference audio files` | 参考素材超过上限 | 减少对应类型的素材数量。 |
| URL 下载失败或被拒绝 | URL 不公开、过期、内网地址或文件过大 | 使用稳定的公网 URL，或改为 multipart 上传。 |
| 参考视频或配套音频处理失败 | 素材超过 3 段、URL 不可访问，或素材无法被解析 | 检查字段、数量和素材可用性；也可改用 multipart 上传。 |

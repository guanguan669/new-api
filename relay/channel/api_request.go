...
// IsUpstreamTransportError reports whether a request reached the HTTP transport
// and then failed while talking to the upstream service. Configuration and
// request-construction errors intentionally return false.
...
func doRequest(c *gin.Context, req *http.Request, info *common.RelayInfo) (*http.Response, error) {
	...
	if err != nil {
		...
		return nil, &upstreamTransportError{err: types.NewError(err, types.ErrorCodeDoRequestFailed, types.ErrOptionWithHideErrMsg("upstream error: do request failed"))}
	}
	if resp == nil {
